...
public class MatchManagement
{
    ...
    {
        var groupedMatches = matches.GroupBy(m => m.Sport);

        foreach (var group in groupedMatches)
        {
            int highestHomeTeamScore = group.Max(m => m.HomeTeamScore);
            int highestAwayTeamScore = group.Max(m => m.AwayTeamScore);

            Console.WriteLine($"Sport: {group.Key}");
            Console.WriteLine($"Highest Home Team Score: {highestHomeTeamScore}");
            Console.WriteLine($"Highest Away Team Score: {highestAwayTeamScore}");
            Console.WriteLine();
        }
    }
    public void DisplayLowestScores()
        {
            var groupedMatches = matches.GroupBy(m => m.Sport);

            foreach (var group in groupedMatches)
            {
                int lowestHomeTeamScore = group.Min(m => m.HomeTeamScore);
                int lowestAwayTeamScore = group.Min(m => m.AwayTeamScore);

                Console.WriteLine($"Sport: {group.Key}");
                Console.WriteLine($"Lowest Home Team Score: {lowestHomeTeamScore}");
                Console.WriteLine($"Lowest Away Team Score: {lowestAwayTeamScore}");
                Console.WriteLine();
            }
        }
    public void CalculateAndDisplayAverageScores()
    {
        var groupedMatches = matches.GroupBy(m => m.Sport);

        foreach (var group in groupedMatches)
        {
            double averageHomeTeamScore = group.Average(m => m.HomeTeamScore);
            double averageAwayTeamScore = group.Average(m => m.AwayTeamScore);

            Console.WriteLine($"Sport: {group.Key}");
            Console.WriteLine($"Average Home Team Score: {averageHomeTeamScore:F2}");
            Console.WriteLine($"Average Away Team Score: {averageAwayTeamScore:F2}");
            Console.WriteLine();
        }
    }
